...
class InceptionV3:
	...
	def _create_inception_graph(self,modelPath):
		with tf.Graph().as_default() as self.inceptionGraph:
			with tf.gfile.FastGFile(modelPath, 'rb') as f:
				...
				self.bottleneckTensor, self.jpeg_data_tensor, resized_input_tensor, self.decoded_jpeg_data_tensor = (tf.import_graph_def(graph_def, name='', return_elements=[config.BOTTLENECK_TENSOR_NAME, config.JPEG_DATA_TENSOR_NAME,config.RESIZED_INPUT_TENSOR_NAME,config.DECODED_JPEG_DATA_TENSOR_NAME]))
	...
	def _add_contrib_bn_fully_connected_layer(self,input_to_layer,input_size,output_size,layer_name,keep_rate,is_training):
		with tf.name_scope(layer_name, reuse=None):
			with tf.name_scope('weights'):
				initial_value_weights = tf.truncated_normal([input_size, output_size],stddev=0.001)
				layer_weights = tf.Variable(initial_value_weights, name='final_weights')
			with tf.name_scope('biases'):
	...
	def add_final_training_ops(self,class_count, final_tensor_name, optimizer_name, num_batches_per_epoch, FLAGS):
		with self.inceptionGraph.as_default():
			with tf.name_scope('input'):
				self.bottleneckInput = tf.placeholder_with_default(self.bottleneckTensor, shape=[None, config.BOTTLENECK_TENSOR_SIZE],name='BottleneckInputPlaceholder')
			...
			logits_final_minus_2 = self._add_fully_connected_layer(self.bottleneckInput,config.BOTTLENECK_TENSOR_SIZE,config.FINAL_MINUS_2_LAYER_SIZE,layer_name,self.keep_rate,self.is_training_ph,FLAGS)

			layer_name = 'final_minus_1_training_ops'
			logits_final_minus_1 = self._add_fully_connected_layer(logits_final_minus_2,config.FINAL_MINUS_2_LAYER_SIZE,config.FINAL_MINUS_1_LAYER_SIZE,layer_name,self.keep_rate,self.is_training_ph,FLAGS)

			layer_name = 'final_training_ops'
			with tf.name_scope(layer_name):
				with tf.name_scope('weights'):
					initial_value = tf.truncated_normal([config.FINAL_MINUS_1_LAYER_SIZE, class_count],stddev=0.001)
	...
	def add_input_distortions(self, flip_left_right, random_crop, random_scale,
	  ...
	  with self.inceptionGraph.as_default():
		  decoded_image_as_float = tf.placeholder('float', [None,None,config.MODEL_INPUT_DEPTH])
		  ...
		  precrop_width = tf.multiply(scale_value, config.MODEL_INPUT_WIDTH)
		  precrop_height = tf.multiply(scale_value, config.MODEL_INPUT_HEIGHT)
		  ...
		  cropped_image = tf.random_crop(precropped_image_3d,
		                                 [config.MODEL_INPUT_HEIGHT, config.MODEL_INPUT_WIDTH,
		                                  config.MODEL_INPUT_DEPTH])
		  if flip_left_right:
